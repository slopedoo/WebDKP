...
function WebDKP_UpdatePlayersInGroup()
	...
	bench_prevNumInRaid = numberInRaid;
	-- Is a raid going?
...
function WebDKP_UpdatePlayersInGroup_PlayerJoined()
	local ret = "";
	local playerNameAdded = true;
	local numberInRaid = GetNumRaidMembers();

	if ( numberInRaid > 0 ) then
		if(bench_prevNumInRaid < numberInRaid) then
			local name, class;
			for i=1, numberInRaid do
				name, _, _, _, class, _, _, _ , _ = GetRaidRosterInfo(i);
				playerNameAdded = true;
				for j=1, bench_prevNumInRaid do
					if ( WebDKP_PlayersInGroup[j]["name"] == name ) then
						playerNameAdded = false;
					end
				end
				if(playerNameAdded) then
					ret = name;
				end
			end
		end
...
function WebDKP_IsInRaid(nameGiven)
	local ret = false;
	local numberInRaid = GetNumRaidMembers();
