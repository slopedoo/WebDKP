...
WebDKP_DkpAwardAll =		"WebDKP: $dkp dkp awarded to all players: $reason.";
...
function WebDKP_AnnounceBidEnd(item, name, dkp)
	...
	--local _,_, link = WebDKP_GetItemInfo(item);
	local tellLocation = WebDKP_GetTellLocation();
	local toSay = string.gsub(WebDKP_BidEnd, "$item", item); -- replace $item in WebDKP_BidEnd string with the item name
